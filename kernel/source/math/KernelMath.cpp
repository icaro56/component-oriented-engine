...
const float Math::InfinityF = std::numeric_limits<float>::max();
...
Math::Math( unsigned int trigTableSize )
{
	...
	mSinTable = OGRE_ALLOC_T(Real, mTrigTableSize, Ogre::MEMCATEGORY_GENERAL);
	mTanTable = OGRE_ALLOC_T(Real, mTrigTableSize, Ogre::MEMCATEGORY_GENERAL);
...
Math::~Math()
{
	OGRE_FREE(mSinTable, Ogre::MEMCATEGORY_GENERAL);
	OGRE_FREE(mTanTable, Ogre::MEMCATEGORY_GENERAL);
}
...
{
	return Real(Ogre::asm_rsq(fValue));
}
//-----------------------------------------------------------------------
Real Math::UnitRandom ()
{
	...
	else return Ogre::asm_rand() / Ogre::asm_rand_max();
}
